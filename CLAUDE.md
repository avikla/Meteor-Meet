...
| `daily-report.gs` | GAS — daily DB usage report to `avi.klayman@gmail.com` at midnight IST. Own GAS project (script ID `1MCoKYf2EVaueAzpjWAmHdvzubUcj3NqLAXzrBic6oRZgxacpnf44uYBD`), pushed from this folder's `.clasp.json`. |
| `appsscript.json` | GAS manifest for the `daily-report.gs` project — OAuth scopes, timezone (Asia/Jerusalem), runtime |
| `gas-cleanup/cleanup.gs` | GAS — private web-app admin page (`doGet`) to review and delete expired events. **Separate GAS project** (script ID `1EsOw-Ur4TdGC5ObVlXyX_ouSvvfm9ByN3TkxaTT0ftTBXLuXLzrOXsEZ`, own `.clasp.json`/`appsscript.json` in `gas-cleanup/`, excluded from the root project's push via `.claspignore`) — split out 2026-09-05 so its `datastore` OAuth scope can't take down the daily-report trigger; see GAS Deployment section and the corrected gotcha in GAS Daily Report. |
...
**Two separate GAS projects as of 2026-09-05** (split from one shared project — see the corrected gotcha in GAS Daily Report for why). They no longer share an `appsscript.json` or an OAuth authorization grant; a scope change or auth expiry in one cannot affect the other.

### daily-report.gs project

`https://script.google.com/d/1MCoKYf2EVaueAzpjWAmHdvzubUcj3NqLAXzrBic6oRZgxacpnf44uYBD/edit` — pushed from `projects/whenfree/` (root `.clasp.json`). `oauthScopes`: `monitoring.read`, `script.scriptapp`, `script.external_request` only — **no** restricted/sensitive scopes since the split.

A plain push is enough — `sendDailyReport`'s time-based trigger runs against `@HEAD`, confirmed 2026-08-06 (manual run + real trigger both picked up a same-day push with no redeploy step):
...
**Gotcha — restricted OAuth scopes silently kill the trigger (root-caused and fixed 2026-08-06):** the daily trigger went dark for 5 consecutive nights (2026-08-01 through 2026-08-06) with zero trace — no execution log entry, no error, no `FAILED` email, no `/fail` ping, trigger still correctly installed and enabled the whole time. Initial hypothesis (2026-08-02, after seeing only one missed night) was a harmless one-off Google-side trigger skip, which is why the healthchecks.io **grace window was widened to 2 days** (Period=2 days, as of 2026-08-02) — that widening is still in place as a sane safety margin, but it was not the real fix. Checking Gmail directly (not just the Executions dashboard) revealed the outage was 5 nights long, not 1, which pointed to the actual cause: this GAS project uses **restricted/sensitive OAuth scopes** (`https://mail.google.com/` via `GmailApp`, `.../auth/datastore`) and has never completed Google's app-verification process, so granted authorization silently expires roughly every 7 days — Google's trigger service can't even invoke the script on an expired grant, which is why there's no execution record at all, not even a failed one. The `oauthScopes` change on 2026-07-25 (adding `datastore`) forced a fresh authorization that then expired ~7 days later, right when the trigger went dark. **Fix (partial, 2026-08-06):** `daily-report.gs` no longer uses `GmailApp` — it sends mail via the ZeptoMail Cloud Function instead (see Email System section), and `https://mail.google.com/` was removed from `oauthScopes` entirely. The `datastore` scope was left in place (needed by `cleanup.gs`, which shared this same project/manifest), on the assumption that its expiry "only affects a manually-opened admin tool, not a silent background trigger."

**That assumption was wrong, and the bug recurred (2026-08-25/26 → fixed 2026-09-05):** Apps Script OAuth authorization is granted per-**project**, not per-function. Any restricted/sensitive scope anywhere in the manifest puts *every* trigger in that project at risk of the same silent expiry, even triggers whose own functions never touch that scope — `daily-report.gs` never calls Firestore via OAuth (its reads use a plain API key), yet the trigger still went dark because `datastore` was declared in the same `appsscript.json` that `cleanup.gs` needed for `deleteEvents()`. Confirmed via `sendMail` Cloud Function request logs: nightly 200 responses from Apps Script every day from Aug 6–25, then zero requests at all (not even a failed one) from Aug 26 onward — proof the trigger died before ever calling out, not that a key or payload broke.

**Real fix:** split `cleanup.gs` into its own separate GAS project (`gas-cleanup/`, see GAS Deployment) so `daily-report.gs`'s project carries zero restricted/sensitive scopes. `cleanup.gs`'s own project still carries `datastore` and can still hit this same expiry — but that's now isolated to a manually-opened admin tool failing visibly ("page won't load"), not a silent nightly outage.

**Watch for:** if the daily trigger goes silent again with the exact same signature (no execution record, healthchecks.io stays Down with no recovery, and `gcloud logging read` on `sendMail` shows zero requests from a Google-Apps-Script user agent), check `daily-report.gs`'s project's `oauthScopes` first — if it's back to zero restricted scopes and this still recurs, the remaining theory is Google's general unverified/"Testing" OAuth consent screen refresh-token expiry, and the durable fix would be either completing app verification or moving the whole job to a Cloud Function + Cloud Scheduler (as `sendMail` already did) to get service-account credentials instead of a user OAuth grant.

**Independent confirmation:** healthchecks.io's own alert (a separate system from the `gcloud logging` evidence used to diagnose this) flipped "WhenFree Daily Report" from Down to Up at 2026-09-05 14:36:29 +0300, reporting 8 days 10 hours of downtime — consistent with the last good automatic run (Aug 25) plus the check's 2-day grace period before it declares Down. This matches the fix timeline and corroborates the diagnosis from a source that doesn't depend on Cloud Function logs at all.
